...
def getGeneCoverage(bamFilesList, geneOfInterest, gtfFile, outDir, metadata):
	# Output file name
	print(metadata)
	outFile = "/data/samples.geneBodyCoverage.txt"
	OUT1 = open(outFile	,'w')
	...
	with open("/data/g_percentiles.json") as json_file:
	...
	for bamfile in bamFilesList:
		bamfile_string = str(bamfile).replace(".bam","").split("/")[-1]
		print(bamfile_string, file=sys.stdout)
		if bamfile_string in list(metadata["Samples"]):
...
metadata = pd.read_csv(metadata_path,header=0,index_col=None,sep="\t")
...
getGeneCoverage(bamFilesList, geneOfInterest, gtf, outDir, metadata)
